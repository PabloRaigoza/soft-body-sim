...
export class SplineModel extends Curve2DModel{
    ...
    getLinearInterpolationPoint(progress:number):Vec2{
         ...
         * `endControlPoint`...
         */
        return startControlPoint.times(1-alpha).plus(endControlPoint.times(alpha));
    ...
    getLinearInterpolationColor(progress:number):Color{
        ...
        let startControlPointIndex = tfloor;
        let startColor = this.verts.color.getAt(startControlPointIndex);

        let endControlPointIndex =Math.ceil(currentIndex);
        let endColor = this.verts.color.getAt(endControlPointIndex);

        let alpha = currentIndex-tfloor;

        return startColor.times(1 - alpha).plus(endColor.times(alpha));
    ...
    getLinearInterpolationVelocity(progress:number):Vec2{
        ...
        const currentIndex = progress * (this.nControlPoints - 2);
        const tfloor = Math.floor(currentIndex);
        ...
        return endControlPoint.minus(startControlPoint).getNormalized();
        // TODO: Replace the line above with your own code
    }
    //</editor-fold>
    //###########################################\\--Linear Interpolation--//###########################################
    ...
    get nInterpolatedControlPoints():number{
        ...
        return Math.floor((this.verts.nVerts) / 3) + 1;
        // TODO: Replace the line above with your own code
    }
    ...
    get nBezierSegments():number{
        if (this.verts.nVerts < 4) {
            return 0;
        }
        return Math.floor((this.verts.nVerts) / 3);
        // TODO: Replace the line above with your own code
    }
    ...
    getCubicBezierInterpolationPoint(progress:number):Vec2{
        ...
        const bezierSegmentCount = this.continuousSegmentIndexForProgress(progress);
        let alpha =  bezierSegmentCount - Math.floor(bezierSegmentCount);
        let [p0, p1, p2, p3] = this.getSegmentControlPoints(Math.floor(bezierSegmentCount));
        if (progress === 1) {
            alpha = 1;
            [p0, p1, p2, p3] = this.getSegmentControlPoints(bezierSegmentCount-1);
        }
    ...
    getCubicBezierInterpolatedColor(progress:number):Color{
        ...
        const bezierSegmentCount = this.continuousSegmentIndexForProgress(progress);
        let alpha = bezierSegmentCount - Math.floor(bezierSegmentCount);
        let [c0, c1, c2, c3] = this.getSegmentControlPointColors(Math.floor(bezierSegmentCount));
        if (progress === 1) {
            alpha = 1;
            [c0, c1, c2, c3] = this.getSegmentControlPointColors(bezierSegmentCount-1);
        }
    ...
    getCubicBezierVelocityVector(progress:number):Vec2{
        ...
        const tangent = GetCubicBezierSplineSegmentDerivativeForAlpha(alpha, p0, p1, p2, p3);
        return tangent;
        // TODO: Replace the line above with your own code
    }
    //</editor-fold>
